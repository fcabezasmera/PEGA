...
score_cols = [c for c in df.columns if c.endswith("_score")
              and c not in ("mean_score","geomean_score","median_score",
...
for col in SUMMARY_COLS:
    check(col in df.columns,                            f"{col} present")

# Verify mathematical correctness on first row
row = df.iloc[0]
raw = row[score_cols].values.astype(float)
check(abs(row["mean_score"]    - np.mean(raw))             < 1e-9, "mean_score correct")
check(abs(row["geomean_score"] - np.exp(np.mean(np.log(raw+1e-9)))) < 1e-9, "geomean_score correct")
check(abs(row["median_score"]  - np.median(raw))           < 1e-9, "median_score correct")
check(abs(row["min_score"]     - np.min(raw))              < 1e-9, "min_score correct")
check(abs(row["std_score"]     - np.std(raw))              < 1e-9, "std_score correct")
expected_cs = 1 / (1 + np.std(raw))
check(abs(row["consensus_score"] - expected_cs)            < 1e-9, "consensus_score correct")

check(all(df["consensus_score"].between(0, 1)),             "consensus_score ∈ [0,1]")
check(all(df["geomean_score"] <= df["mean_score"] + 1e-9), "geomean ≤ mean (always)")

print("\n  ── compute_fitness ──")
df2 = df.drop(columns=["mean_geometric"])
df2 = compute_fitness(df2)
check("mean_geometric" in df2.columns,  "recomputed from geomean_score")
check(all(abs(df2["mean_geometric"] - df["geomean_score"]) < 1e-9),
      "mean_geometric == geomean_score")
...
if not fasta_path.exists():
    ...
    score_cols = [c for c in result.columns if c.endswith("_score")
                  and c not in ("mean_score","geomean_score","median_score",
                                "min_score","std_score","consensus_score")]
    summary_cols = [c for c in ("mean_score","geomean_score","median_score",
                                 "min_score","std_score","consensus_score")
    ...
    check(len(summary_cols) == 6,         f"all 6 summary cols: {summary_cols}")
    check(all(result["geomean_score"] <= result["mean_score"] + 1e-9),
          "geomean ≤ mean for all sequences")
    check(all(result["consensus_score"].between(0, 1)),
          "consensus_score ∈ [0,1] for all")
    check(all(result["min_score"] <= result["mean_score"] + 1e-9),
          "min ≤ mean for all")

    print(f"\n  Preview (first 3 rows):")
    pd.set_option("display.max_columns", 6)
    pd.set_option("display.width", 120)
    print(result[["seq_name"] + score_cols[:3] + ["mean_score","geomean_score","consensus_score"]]
